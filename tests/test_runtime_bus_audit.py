...
class RuntimeBusAuditTests(unittest.TestCase):
    def payload(self, **overrides):
        raw = {
            ...
            "dconf_introspect": True,
            "dconf_detail": "Introspect riuscito",
            "warnings": [],
        }
    ...
    def test_valid_report_is_parsed(self):
        ...
        self.assertTrue(report.dconf_introspect)
        self.assertEqual(report.dconf_detail, "Introspect riuscito")
    ...
    def test_wrong_schema_is_rejected(self):
        with self.assertRaisesRegex(RuntimeError, "schema"):
            rba.parse_runtime_bus_audit_output(self.output(schema=2))
    ...
    def test_non_bool_introspect_is_rejected(self):
        with self.assertRaisesRegex(RuntimeError, "dconf_introspect"):
            rba.parse_runtime_bus_audit_output(self.output(dconf_introspect="yes"))
    ...
    def test_formatter_stays_diagnostic_and_method_specific(self):
        report = rba.parse_runtime_bus_audit_output(self.output())
        text = rba.format_runtime_bus_audit(report)
        self.assertIn("D-Bus system: well-known=2 unique=2", text)
        self.assertIn("ca.desrt.dconf: Introspect consentita", text)
        self.assertNotIn("TALK consentito", text)
