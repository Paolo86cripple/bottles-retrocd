...
def _socket_inode(path: Path) -> str:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"Socket helper Bubblejail non identificabile in modo univoco "
...
def find_instance_namespace_pid(instance: str) -> int:
    ...
    for ns_name in ("user", "mnt", "pid", "net"):
...
def _host_sysfs_binding(name: str) -> tuple[str, Path]:
    ...
    try:
        ...
        raise RuntimeError(f"Gerarchia sysfs gamepad inattesa per {name}: {resolved}") from exc
    root = _validate_sysfs_root(root)
    if os.path.isabs(link_target):
        raise RuntimeError(f"Symlink sysfs assoluto inatteso per {name}: {link_target}")
...
def _bind_fd(target: Path, source_fd: int) -> None:
...
def _bind_device_fd(name: str, source_fd: int) -> None:
    name = _validate_node_name(name)
    input_dir = Path("/dev/input")
    input_dir.mkdir(mode=0o755, parents=True, exist_ok=True)
    target = input_dir / name
    _bind_fd(target, source_fd)
    print(f"BOUND={name}", flush=True)


def _bind_sysfs_fd(target: Path, source_fd: int) -> None:
...
def _install_sysfs_link(name: str, link_target: str) -> None:
    ...
    if not link_target or os.path.isabs(link_target):
        raise RuntimeError(f"Target symlink sysfs rifiutato per {name}: {link_target!r}")
    ...
    try:
        ...
        raise RuntimeError(f"Target sysfs {target} non è un symlink gestibile: {exc}") from exc
...
def _state_dir() -> Path:
    return Path("/run/user") / str(os.getuid()) / "bottles-retro-cd-gamepad-hotplug"
...
def _load_state(name: str) -> tuple[Path, str, dict[str, str]]:
    ...
    if not isinstance(link_target, str) or not link_target or os.path.isabs(link_target):
    ...
    for key, value in props.items():
        if not isinstance(key, str) or not isinstance(value, str):
            raise RuntimeError(f"Stato hotplug: proprietà udev non stringa per {name}.")
...
def _udev_packet(properties: dict[str, str], action: str) -> bytes:
    ...
    for key in ordered:
        ...
        chunks.append(f"{key}={value}".encode("utf-8", errors="strict") + b"\0")
...
def _mutate_inside_namespace(
    device_fds: dict[str, int],
    sysfs_fds: dict[Path, int],
    ...
    Path("/sys/class/input").mkdir(mode=0o755, parents=True, exist_ok=True)
    ...
    for name in sorted(removed):
        ...
        _drop_state(name)
    for root in sorted(removed_roots, key=lambda p: (len(p.parts), str(p)), reverse=True):
        _umount_all(root)
        print(f"SYSFS_UNBOUND={root}", flush=True)

    for root in sorted(sysfs_fds, key=str):
        _bind_sysfs_fd(root, sysfs_fds[root])
    for name in sorted(sysfs_links):
        link_target, _ = sysfs_links[name]
        _install_sysfs_link(name, link_target)
    for name in sorted(device_fds):
        _bind_device_fd(name, device_fds[name])
...
def mutate_instance(
    ...
    *,
    ...
    sysfs_fds: dict[Path, int] = {}
    sysfs_links: dict[str, tuple[str, Path]] = {}
    user_fd = mnt_fd = pid_fd = net_fd = -1
    try:
        for name, path, link_target, sysfs_root in validated:
            device_fds[name] = os.open(path, os.O_PATH | os.O_CLOEXEC)
            sysfs_links[name] = (link_target, sysfs_root)
            if sysfs_root not in sysfs_fds:
                sysfs_fds[sysfs_root] = os.open(
        ...
        pid_fd = os.open(
            f"/proc/{namespace_pid}/ns/pid", os.O_RDONLY | os.O_CLOEXEC
        )
        net_fd = os.open(
        ...
        need_pid = os.readlink("/proc/self/ns/pid") != os.readlink(
            f"/proc/{namespace_pid}/ns/pid"
        )
        ...
        if not os.WIFEXITED(status) or os.WEXITSTATUS(status) != 0:
            ...
            raise RuntimeError(
                f"Mutazione mount/udev namespace fallita nel child (rc={code})."
            )
    finally:
        for fd in device_fds.values():
            try:
                os.close(fd)
            except OSError:
                pass
        for fd in sysfs_fds.values():
            try:
                os.close(fd)
            except OSError:
                pass
        for fd in (user_fd, mnt_fd, pid_fd, net_fd):
            if fd >= 0:
                try:
                    os.close(fd)
                except OSError:
